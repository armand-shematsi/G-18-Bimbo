    ...
    <div class="min-h-screen bg-gray-100 flex">
        ...
        <aside id="sidebar" class="hidden md:flex flex-col w-64 bg-blue-600 text-white border-r border-gray-200 min-h-screen sticky md:static top-0 z-40 transition-all duration-300">
            ...
            <nav class="flex-1 px-4 py-6 space-y-2">
                <a href="{{ route('bakery.production') }}" class="block px-4 py-2 rounded-lg {{ request()->routeIs('bakery.production') ? 'bg-sky-100 text-sky-700 font-bold' : 'text-gray-700 hover:bg-sky-50' }}">Production Monitoring</a>
                <a href="{{ route('workforce.overview', array_filter(['supply_center_id' => request('supply_center_id')])) }}" class="block px-4 py-2 rounded-lg {{ request()->routeIs('workforce.overview') ? 'bg-sky-100 text-sky-700 font-bold' : 'text-gray-700 hover:bg-sky-50' }}">Workforce Distribution Management</a>
                <a href="{{ route('bakery.inventory.index') }}" class="block px-4 py-2 rounded-lg {{ request()->routeIs('bakery.inventory.*') ? 'bg-sky-100 text-sky-700 font-bold' : 'text-gray-700 hover:bg-sky-50' }}">Inventory Management</a>
                <a href="{{ url('/supplier/raw-materials/catalog') }}" class="block px-4 py-2 rounded-lg {{ request()->is('supplier/raw-materials/catalog') ? 'bg-green-100 text-green-700 font-bold' : 'text-green-600 hover:bg-green-50' }}">Order Raw Materials</a>
                <a href="{{ route('bakery.maintenance') }}" class="block px-4 py-2 rounded-lg {{ request()->routeIs('bakery.maintenance') ? 'bg-sky-100 text-sky-700 font-bold' : 'text-gray-700 hover:bg-sky-50' }}">Machine Maintenance</a>
                <a href="{{ route('bakery.order-processing') }}" class="block px-4 py-2 rounded-lg {{ request()->routeIs('bakery.order-processing') ? 'bg-sky-100 text-sky-700 font-bold' : 'text-gray-700 hover:bg-sky-50' }}">Order Processing</a>
            </nav>
        </aside>
        <!-- Main Content Area -->

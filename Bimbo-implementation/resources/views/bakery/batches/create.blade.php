...
<div class="max-w-xl mx-auto">
    <form method="POST" action="{{ route('bakery.batches.store') }}">
        ...
        <div>
            ...
            <input type="date" name="actual_start_date" id="actual_start_date" value="{{ old('actual_start_date') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
        </div>
        <div>
            <label for="actual_start_time_raw" class="block text-sm font-medium text-gray-700">Actual Start Time</label>
            <div class="flex space-x-2">
                <input type="time" name="actual_start_time_raw" id="actual_start_time_raw" value="{{ old('actual_start_time_raw') }}" class="block w-full rounded-md border-gray-300 shadow-sm">
                <select name="actual_start_time_ampm" id="actual_start_time_ampm" class="block rounded-md border-gray-300 shadow-sm">
                    <option value="AM" {{ old('actual_start_time_ampm') == 'AM' ? ' selected' : '' }}>AM</option>
                    <option value="PM" {{ old('actual_start_time_ampm') == 'PM' ? ' selected' : '' }}>PM</option>
                </select>
            </div>
        ...
        <div>
            ...
            <input type="date" name="actual_end_date" id="actual_end_date" value="{{ old('actual_end_date') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
        </div>
        <div>
            <label for="actual_end_time_raw" class="block text-sm font-medium text-gray-700">Actual End Time</label>
            <div class="flex space-x-2">
                <input type="time" name="actual_end_time_raw" id="actual_end_time_raw" value="{{ old('actual_end_time_raw') }}" class="block w-full rounded-md border-gray-300 shadow-sm">
                <select name="actual_end_time_ampm" id="actual_end_time_ampm" class="block rounded-md border-gray-300 shadow-sm">
                    <option value="AM" {{ old('actual_end_time_ampm') == 'AM' ? ' selected' : '' }}>AM</option>
                    <option value="PM" {{ old('actual_end_time_ampm') == 'PM' ? ' selected' : '' }}>PM</option>
                </select>
            </div>
        ...
        <div class="mb-4">
            ...
            <input type="number" name="quantity" id="quantity" class="mt-1 block w-full border-gray-300 rounded-md" min="0" value="{{ old('quantity') }}">
        </div>
        <div class="mb-4">
